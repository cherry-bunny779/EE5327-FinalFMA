...
module dp_fma_pipelined (
    ...
    output logic        valid_out        // Output valid signal (directly registered)
);
    ...
    localparam WORK_WIDTH = 161;  // 106 + 53 + 2 guard bits
    
    //==========================================================================
    // Pipeline Valid Signals
    // Note: 3 pipeline registers = 3 cycle latency
    //   Cycle 1: valid_s1 (after Stage 1 register)
    //   Cycle 2: valid_s2 (after Stage 2 register)
    //   Cycle 3: valid_s3 (after Stage 3/Output register)
    //==========================================================================
    ...
    // valid_out is registered in the output register block (Stage 3)
    ...
    assign mantissa_c_full = c_is_zero ? 53'b0 :
    ...
    logic signed [12:0]    e_aligned_s2;
    ...
    assign sticky = |sum_normalized[WORK_WIDTH-56:0] | sticky_bit_align_s2;
    ...
    always_comb begin
        ...
        if (a_is_nan_s2 || b_is_nan_s2 || c_is_nan_s2) begin
            ...
            // For c_is_zero case, we use the computed result directly
            // since we don't have mult_result in pipelined version
            if (e_final_biased <= 0) begin
                s_out = s_result_tmp_s2;
                e_out = 11'h000;
                m_out = 52'b0;
                underflow_comb = 1'b1;
            end else if (e_final_biased >= 2047) begin
                s_out = s_result_tmp_s2;
                e_out = 11'h7FF;
                m_out = 52'b0;
                overflow_comb = 1'b1;
    ...
    //==========================================================================
    // STAGE 3 Output Registers (This is the 3rd pipeline stage)
    //==========================================================================
